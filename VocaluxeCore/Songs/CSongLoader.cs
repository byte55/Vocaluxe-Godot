    ...
    public partial class CSong
    {
        ...
        private class CSongLoader
        {
            ...
            public bool InitPaths(string filePath)
            {
                ...
                // Original derived FolderName relative to the configured song roots (CBase.Config.GetSongFolders()).
                // The portable core has no song-folder config, so we just use the containing folder name.
                _Song.FolderName = Path.GetFileName(_Song.Folder);
            ...
            public bool ReadHeader(bool useSetEncoding = false)
            {
                ...
                try
                {
                    ...
                    while (!sr.EndOfStream)
                    {
                        ...
                        var identifier = line.Substring(1, pos - 1).Trim().ToUpper();
                        ...
                        {
                            case "ENCODING":
                                var newEncoding = value.GetEncoding();
                            ...
                            case "INSTRUMENTAL":
                                ...
                                {
                                    CLog.CSongLog.Error("[{SongFileName}] Can't find instrumental audio file: {AudioFile}",
                                        CLog.Params(_Song.FileName, Path.Combine(_Song.Folder, value)));
                                    return false;
                                }
                            ...
                            case "VOCALS":
                                ...
                                {
                                    CLog.CSongLog.Error("[{SongFileName}] Can't find vocal audio file: {AudioFile}",
                                        CLog.Params(_Song.FileName, Path.Combine(_Song.Folder, value)));
                                    return false;
                                }
                            ...
                            case "BACKGROUND":
                                ...
                                {
                                    CLog.CSongLog.Warning("[{SongFileName}] Can't find background file: {MissingFile}",
                                        CLog.Params(_Song.FileName, Path.Combine(_Song.Folder, value)));
                                }
                            ...
                            case "ENDSHORT":
                                if ((headerFlags & EHeaderFlags.Bpm) != 0)
                                {
                                    ...
                                    {
                                        _Song.ShortEnd.EndBeat = (int)CGame.GetBeatFromTime(endTime / 1000f, _Song.Bpm, _Song.Gap);
                            ...
                            default:
                                ...
                                if (identifier.StartsWith("P"))
                                {
                                    ...
                                    {
                                        foreach (var curPlayer in player.GetSetBits())
                                        {
                    ...
                    if ((headerFlags & EHeaderFlags.MedleyStartBeat) != 0 && (headerFlags & EHeaderFlags.MedleyEndBeat) != 0)
                    {
                        ...
                        {
                            CLog.CSongLog.Error("[{SongFileName}] MedleyStartBeat is bigger than MedleyEndBeat in file: {StartBeat} > {EndBeat}",
                                CLog.Params(_Song.FileName, _Song.Medley.StartBeat > _Song.Medley.EndBeat));
                    ...
                    if (_Song.Preview.Source == EDataSource.None)
                    {
                        ...
                        _Song.Preview.StartTime = (headerFlags & EHeaderFlags.MedleyStartBeat) != 0 ? CGame.GetTimeFromBeats(_Song.Medley.StartBeat, _Song.Bpm) : 0f;
                    ...
                    if ((headerFlags & EHeaderFlags.MedleyStartBeat) != 0 && (headerFlags & EHeaderFlags.MedleyEndBeat) != 0)
                    {
                        ...
                        _Song.Medley.FadeInTime = CSettings.DefaultMedleyFadeInTime;
                        _Song.Medley.FadeOutTime = CSettings.DefaultMedleyFadeOutTime;
                    }
                ...
                // TODO(godot-rework): play-count/date come from the highscore DB (CBase.DataBase) in the app.
                // Wire this up once the DB layer is ported; defaults are fine for loading.
            ...
            private class CAutoChanges
            {
                ...
                {
                    var result = "";
                    var skippedNotesCt = ZeroLengthNoteCt + OverlapNoteCt + NoTextNoteCt;
                    if (skippedNotesCt > 0)
                    {
                    ...
                    var adjustedBreakCt = AjustedBreakCt + NoLengthBreakCt;
            ...
            public bool ReadNotes(bool forceReload = false)
            {
                ...
                var currentBeat = 0; //Used for relative songs
                CSongNote lastNote = null; //Holds last parsed note. Get's reset on player change
                var endFound = false; // True if end tag was found
                ...
                try
                {
                    ...
                    while (!sr.EndOfStream && !endFound)
                    {
                        ...
                        switch (tag)
                        {
                            ...
                            case 'G':
                            case 'F':
                                var noteData = line.Split(splitChars, 4);
                                ...
                                {
                                    CLog.CSongLog.Error("[{SongFileName}] Invalid note found (non-numeric values) (in line {LineNr}): {noteData}",
                                        CLog.Params(_Song.FileName, _LineNr, noteData));
                                    sr.Dispose();
                                    return false;
                                }

                                var text = noteData[3].TrimMultipleWs();
                                if (text == "")
                                {
                                    CLog.CSongLog.Warning("[{SongFileName}] Ignored note without text (in line {LineNr})", CLog.Params(_Song.FileName, _LineNr));
                                    changesMade.NoTextNoteCt++;
                                ...
                                if (length < 1)
                                {
                                    ...
                                    {
                                        CLog.CSongLog.Warning("[{SongFileName}] Found more than {MaxZeroNoteCt} notes with length < 1. Trying alternative read mode.",
                                            CLog.Params(_Song.FileName, _MaxZeroNoteCt));
                                    ...
                                    foreach (var curPlayer in player.GetSetBits())
                                    {
                                        ...
                                        if (!_AddNote(curPlayer, lastNote))
                                        {
                                            if (!ignored)
                                            {
                                                ...
                                                {
                                                    CLog.CSongLog.Warning("[{SongFileName}] Found more than {MaxOverlapNoteCt} overlapping notes. Using standard mode.",
                                                        CLog.Params(_Song.FileName, _MaxOverlapNoteCt));
                                            ...
                                            CLog.CSongLog.Warning(
                                                "[{SongFileName}] Ignored note for player {CurrentPlayerNumber} because it overlaps with other note (in line {LineNr})",
                                                CLog.Params(_Song.FileName, curPlayer + 1, _LineNr));
                                        }
                                    }
                                }
                                ...
                                break;
                            case '-':
                                var lineBreakData = line.Split(splitChars);
                                if (lineBreakData.Length < 1)
                                {
                                    CLog.CSongLog.Error("[{SongFileName}] Invalid line break found (No beat) (in line {LineNr}): {LineBreakData}",
                                        CLog.Params(_Song.FileName, _LineNr, lineBreakData));
                                    sr.Dispose();
                                    return false;
                                }
                                ...
                                {
                                    CLog.CSongLog.Error("[{SongFileName}] Invalid line break found (Non-numeric value) (in line {LineNr}): {LineBreakData}",
                                        CLog.Params(_Song.FileName, _LineNr, lineBreakData));
                                    sr.Dispose();
                                    return false;
                                }
                                ...
                                if (_Song.Relative)
                                {
                                    ...
                                    {
                                        CLog.CSongLog.Warning("[{SongFileName}] Missing line break length (in line {LineNr}):{LineBreakData}",
                                            CLog.Params(_Song.FileName, _LineNr, lineBreakData));
                                ...
                                {
                                    CLog.CSongLog.Warning("[{SongFileName}] Line break is before previous note end. Adjusted. (in line {LineNr})",
                                        CLog.Params(_Song.FileName, _LineNr));
                                ...
                                else
                                {
                                    foreach (var curPlayer in player.GetSetBits())
                                    {
                                        ...
                                        {
                                            CLog.CSongLog.Warning("[{SongFileName}] Ignored line break for player {CurPlayerNr} (Overlapping or duplicate) (in line {LineNr})",
                                                CLog.Params(_Song.FileName, curPlayer + 1, _LineNr));
                                        }
                                    }
                                }
                    ...
                    for (var i = 0; i < _Song.Notes.VoiceCount; i++)
                    {
                        ...
                        var emptyLines = voice.RemoveEmptyLines();
                        if (emptyLines > 0)
                        {
                            CLog.CSongLog.Warning("[{SongFileName}] Removed {NumEmptyLines} empty lines from P .This often indicates a problem with the line breaks in the file",
                                CLog.Params(_Song.FileName, emptyLines));
                        }
                ...
                if (changesMade.IsModified)
                {
                    ...
                    // TODO(godot-rework): the app optionally saves a ".fix" copy here (CBase.Config.GetSaveModifiedSongs()).
                    // Re-enable once CSongWriter + config are ported.
                }
            ...
            {
                var voice = _Song.Notes.GetVoice(player, true);
